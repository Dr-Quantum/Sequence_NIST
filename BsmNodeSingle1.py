...
def runSimulations( _distance, _memory_params, _detector_params, _qchannel_params, _repetitions ):
    ...
    for i in range(3):
        for j in range(3):
            ...
            #newversion
            cc = ClassicalChannel( 'cc_%s_%s' % (nodes[i].name, nodes[j].name),
                                   tl, distance = abs(i-j)*_distance/(len(nodes)-1) )
...
def doSimulationNoRepeater( _attenuation, _max_distance_km, _coherence_time, _mem_eff, _det_eff, _N ):
    ...
    Ls = np.concatenate( ( np.array( [ 1 ] ),
                           np.arange( 10, _max_distance_km + 1, 5 ) ) ) * 1e3
...
def plotGeneratedFiles( _attenuation, _coherence_time, _mem_eff, _det_eff, _N, _saveFig ):
    ...
    fname1 = ( 'entRate(att=%.3fdBkm,cohTime=%s%smemEff=%.2f,detEff=%.2f,N=1E%d).txt' 
                % ( 
                    _attenuation * 1000,
                     '%.3fms' % ( 0.1e-3 / 1e-3 ) if 0.1e-3 > 0.0 else 'inf',
                     ( '+-%.2fmcs,' % ( _coherence_time_stdev / 1e-6 )
                       if ( 0.1e-3 > 0 and _coherence_time_stdev > 0 )
                       else ',' ),
                     _mem_eff, _det_eff, np.log10(_N) ) )
    ...
    plt.semilogy( data1[ :, 0 ]/1000, data1[ :, 1 ], #mec = 'none',
                    label = r'$\alpha$ = %.1f dB/km, $t_\mathrm{coh}$ = %s' % (
                        _attenuation * 1000,
                        r'%s%s' % ( '%.0f' % ( 0.1e-3 / 1e-6 )
                                     if 0.1e-3 > 0 else '\infty',
                                     ( ' $\pm$ %.0f mcs' % ( _coherence_time_stdev / 1e-6 )
                                       if ( False)#0.1e-3 > 0 and _coherence_time_stdev > 0  )
                                       else ' mcs' ) ) ) )
    # _attenuation = _attenuation

    fname5 = ( 'entRate(att=%.3fdBkm,cohTime=%s%smemEff=%.2f,detEff=%.2f,N=1E%d).txt' 
                 % ( 
                     _attenuation * 1000,
                     '%.3fms' % ( 0.2e-3 / 1e-3 ) if 0.2e-3 > 0.0 else 'inf',
                     ( '+-%.2fmcs,' % ( _coherence_time_stdev / 1e-6 )
                       if ( 0.2e-3 > 0 and _coherence_time_stdev > 0 )
                       else ',' ),
                     _mem_eff, _det_eff, np.log10(_N) ) )
    data5 = np.loadtxt( fname5 )
    reg5 = data5[ :, 1 ] > 0.0
    data5 = data5[ reg5, : ]

    plt.semilogy( data5[ :, 0 ]/1000, data5[ :, 1 ], #mec = 'none',
                      label = r'$\alpha$ = %.1f dB/km, $t_\mathrm{coh}$ = %s' % (
                          _attenuation * 1000,
                          r'%s%s' % ( '%.0f' % ( 0.2e-3 / 1e-6 )
                                     if 0.2e-3 > 0 else '\infty',
                                     ( ' $\pm$ %.0f mcs' % ( _coherence_time_stdev / 1e-6 )
                                       if ( False)#0.2e-3 > 0 and _coherence_time_stdev > 0  )
                                       else ' mcs' ) ) ) )
    
    fname2 = ( 'entRate(att=%.3fdBkm,cohTime=%s%smemEff=%.2f,detEff=%.2f,N=1E%d).txt' 
                 % ( 
                     _attenuation * 1000,
                     '%.3fms' % ( 0.4e-3 / 1e-3 ) if 0.4e-3 > 0.0 else 'inf',
                     ( '+-%.2fmcs,' % ( _coherence_time_stdev / 1e-6 )
                       if ( 0.4e-3 > 0 and _coherence_time_stdev > 0 )
                       else ',' ),
                     _mem_eff, _det_eff, np.log10(_N) ) )
    ...
    plt.semilogy( data2[ :, 0 ]/1000, data2[ :, 1 ], #mec = 'none',
                    label = r'$\alpha$ = %.1f dB/km, $t_\mathrm{coh}$ = %s' % (
                        _attenuation * 1000,
                        r'%s%s' % ( '%.0f' % ( 0.4e-3 / 1e-6 )
                                     if 0.4e-3 > 0 else '\infty',
                                     ( ' $\pm$ %.0f mcs' % ( _coherence_time_stdev / 1e-6 )
                                       if ( False)#0.4e-3 > 0 and _coherence_time_stdev > 0  )
                                       else ' mcs' ) ) ) )

    # 0.1e-3 = _coherence_time_avg
    # _attenuation = 0.0
    fname3 = ( 'entRate(att=%.3fdBkm,cohTime=%s%smemEff=%.2f,detEff=%.2f,N=1E%d).txt' 
                 % ( 
                     _attenuation * 1000,
                     '%.3fms' % ( 0.6e-3 / 1e-3 ) if 0.6e-3 > 0.0 else 'inf',
                     ( '+-%.2fmcs,' % ( _coherence_time_stdev / 1e-6 )
                       if ( 0.6e-3 > 0 and _coherence_time_stdev > 0 )
                       else ',' ),
                     _mem_eff, _det_eff, np.log10(_N) ) )
    ...
    plt.semilogy( data3[ :, 0 ]/1000, data3[ :, 1 ], #mec = 'none',
                    label = r'$\alpha$ = %.1f dB/km, $t_\mathrm{coh}$ = %s' % (
                        _attenuation * 1000,
                        r'%s%s' % ( '%.0f' % ( 0.6e-3 / 1e-6 )
                                     if 0.6e-3 > 0 else '\infty',
                                     ( ' $\pm$ %.0f mcs' % ( _coherence_time_stdev / 1e-6 )
                                       if ( False)#0.6e-3 > 0 and _coherence_time_stdev > 0  )
                                       else ' mcs' ) ) ) )
    ...
    fname4 = ( 'entRate(att=%.3fdBkm,cohTime=%s%smemEff=%.2f,detEff=%.2f,N=1E%d).txt' 
                 % ( 
                     _attenuation * 1000,
                     '%.3fms' % ( 0.8e-3 / 1e-3 ) if 0.8e-3 > 0.0 else 'inf',
                     ( '+-%.2fmcs,' % ( _coherence_time_stdev / 1e-6 )
                       if ( 0.8e-3 > 0 and _coherence_time_stdev > 0 )
                       else ',' ),
                     _mem_eff, _det_eff, np.log10(_N) ) )
    ...
    plt.semilogy( data4[ :, 0 ]/1000, data4[ :, 1 ], #mec = 'none',
                      label = r'$\alpha$ = %.1f dB/km, $t_\mathrm{coh}$ = %s' % (
                          _attenuation * 1000,
                          r'%s%s' % ( '%.0f' % ( 0.8e-3 / 1e-6 )
                                     if 0.8e-3 > 0 else '\infty',
                                     ( ' $\pm$ %.0f mcs' % ( _coherence_time_stdev / 1e-6 )
                                       if ( False)#0.8e-3 > 0 and _coherence_time_stdev > 0  )
                                       else ' mcs' ) ) ) )
...
if doSimulations:
    ...
    if generateFiles:
        """
        doSimulationNoRepeater( 0.0, 500,
                                ( -1, __coherence_time_stdev ),
                                __mem_eff, __det_eff, __N )
        doSimulationNoRepeater( 0.0, 500,
                                ( 0.1e-3, __coherence_time_stdev ),
                                __mem_eff, __det_eff, __N )
        """
        doSimulationNoRepeater( __attenuation, 500,
                                ( 0.2e-3, __coherence_time_stdev ),
                                __mem_eff, __det_eff, __N )
        ...
        doSimulationNoRepeater( __attenuation, 500,
                                ( 0.4e-3, __coherence_time_stdev ),
                                __mem_eff, __det_eff, __N )
        doSimulationNoRepeater( __attenuation, 500,
                                ( 0.6e-3, __coherence_time_stdev ),
                                __mem_eff, __det_eff, __N )
        doSimulationNoRepeater( __attenuation, 500,
                                ( 0.8e-3, __coherence_time_stdev ),
                                __mem_eff, __det_eff, __N )
