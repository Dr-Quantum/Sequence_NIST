...
class SimpleManager():
    ...
    def update_eg( self, protocol, memory, state ):
        ...
        if _updated_link.genProtocolFinished:
            ...
            ########### some difference between independent and syncronous modes is here ########################    
                
            if len( self.nodesTracker.links ) > 0:
                _all_protocols_finished = True
                for _l in self.nodesTracker.links:
                    if not _l.genProtocolFinished:
                        _all_protocols_finished = False
                        break
            
                if not _all_protocols_finished:
                    return
                
                #self.nodesTracker.flushAllPendingEvents() #maybe it's not necessary to call it
                ...
                ############## and in the processing of cnode message we should remove this scheduling of further
                # generation trigger, if compared to the syncronous case
            ...
            if msg.msg_type is ControlNodeMsgType.LINK_STATUS_UPDATED:
                if msg.origin == 'generation':
                    ...
                    if _cnodeData.genLinksEstablished == _nodesTracker.numRouters + 1:
                        ...
                        _nodesTracker.tl.schedule( Event( _nodesTracker.tl.now() + 
                                                         _nodesTracker.signalingDelay + 1, #0.5*_nodesTracker.endnode_distance/_nodesTracker.light_speed, 
                                                         _process ) )
                        #else:
                        #    for j in range( 1, _nodesTracker.numRouters + 1 ):
                        #        _nodes[2*j-2].protocols[1].status = GenProtoStatus.INITIALIZED
                        ...
                        #    _cnodeData.numLinksToBeUpdatedAtThisSession = len( _nodesTracker.links )
                        #    #_nodesTracker.resetAllMemories()
                        #    #_nodesTracker.flushTotallyAllPendingEvents()
                        #    #_nodesTracker.result = False
                        #    _have_expired_memories = False
                        #    for _l in _nodesTracker.links:
                        #        if _l.memoryExpired == True:
                        #            _have_expired_memories = True
                        #            break
                        #    _time_to_wait = ( ( 0.55 if _have_expired_memories else 0.5 ) 
                        #                     * _nodesTracker.endnode_distance / _nodesTracker.light_speed 
                        #                     / ( _nodesTracker.numRouters + 1 )  ) 
                        #                        # 0.5+ is because once 
                        #                        #memory expired, it voids emitted photon, but if it had reached bsm node and the bsm 
                        #                        #sent the measurements results, nobody voids that message, so we just have to wait 
                        #                        #once it's delivered and ignored by resource_manager.update() due to the protocol's 
                        #                        #state INITIALIZED
                        #    
                        #    _nodesTracker.tl.schedule( Event( _nodesTracker.tl.now() + _time_to_wait, _nodesTracker.process ) )                            
...
def runSimulations( _numRouters, _distance, _light_speed, _memory_params, _detector_params, _qchannel_params, _swapping_params, _repetitions ):
    ...
    for i in range( len( nodes ) ):
        for j in range( len( nodes ) ):
            ...
            cc = ClassicalChannel( 'cc_%s_%s' % (nodes[i].name, nodes[j].name),
                                   tl, abs(i-j)*_fiberPieceLength, -1, _light_speed )
    ...
    #for i in range( _repetitions ):
        ...
        _time_to_wait = nodesTracker.signalingDelay + 1 #0.5 * _distance / _light_speed
...
def calculate( _numRouters, _max_distance_km, _distance_step_km, _light_speed, _attenuation, _coherence_time,
    ...
    Ls = np.concatenate( ( np.array( [ 1 ] ),
                           np.arange( _distance_step_km if _distance_step_km > 1.0 else ( 1.0 + _distance_step_km ),
                                      _max_distance_km + 0.5*_distance_step_km, _distance_step_km ) ) ) * 1e3
    ...
    fname = dstDir + '/' + ( 'entRateInd(R=%d)(att=%.3fdBkm,cohTime=%s%smemEff=%.2f,detEff=%.2f,swEff=%.2f,N=1E%d).txt' 
                % ( _numRouters,
                    _attenuation * 1000,
...
def plotGeneratedFiles( _attenuation, _coherence_time, _mem_eff, _det_eff, _N, _saveFig ):
    ...
    for _numRouters in range( 9 ):
        ...
        fname2 = ( 'entRateInd(R=%d)(att=%.3fdBkm,cohTime=%s%smemEff=%.2f,detEff=%.2f,N=1E%d).txt' 
                     % (
                         _numRouters,
                         _attenuation1 * 1000,
                         '%.3fms' % ( _coherence_time_avg1 / 1e-3 ) if _coherence_time_avg1 > 0.0 else 'inf',
    ...
    if _saveFig:
        plt.savefig(
            ( 'entRateInd(att=%.3fdBkm,cohTime=%s%smemEff=%.2f,detEff=%.2f,N=1E%d).pdf' 
                 % ( 0.2,
                     '%.3fms' % ( _coherence_time_avg / 1e-3 ) if _coherence_time_avg > 0.0 else 'inf',
                     ( '+-%.2fmcs,' % ( _coherence_time_stdev / 1e-6 )
                       if ( _coherence_time_avg > 0  and _coherence_time_stdev > 0  )
...
if doSimulations:
    ...
    mem_eff = 0.9 #3
    det_eff = 0.8
    swap_probability = 0.5 #5
    swap_degradation = 0.95
    max_distance_km = 12
    distance_step_km = 0.25
    light_speed = 2e-4
    numCoresToUsee = 45#51
    ...
    if generateFiles:
        for ps in pl:
            for _numRepeaters in [ 1, 3, 7 ]: #range( 0, 8 ):
                calculate( _numRepeaters, ps.max_distance_km, ps.light_speed, ps.attenuation,
