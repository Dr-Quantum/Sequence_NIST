...
def drawPanel( _ax, _memTime ):
    ...
    for _numRouters in [ 1, 3, 7
        ...
        ents = entRateInd( Ls*1000, _attenuation1, mem_eff, det_eff, swap_probability, _numRouters )
...
for i in range( len( memTimes ) ):
    ...
    memTime = memTimes[i] if type( memTimes[i] ) is tuple else ( memTimes[i], 0.0 )
    drawPanel( axes[-1], memTime )
    ...
    annStdStr = r'$\pm$%.2f' % (memTimes[i][1]/1e-3) if type( memTimes[i] ) is tuple else ''
